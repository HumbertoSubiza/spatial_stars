...
### 4. Outras ferramentas de extração de datos

Extração usando a função _st_crop()_ da biblioteca **sf**. Foi colocado no seguinte fragmento de código duos formas diferentes, usando o polígono já definido anteriormente ou estabelecendo os límites do corte en forma explícita com um vetor numérico. Esta função só pode ser usada com formas retangulares.

---

```{r 2_14}
p <- st_crop(img[, , , 1:3], pol)
# o polígono também pode ser determinado com xmin, ymin, xmax, ymax da forma:
...
A función _cut_ corta a imagem, classificando os valores em categorias, por exemplo, no fragmento de código a seguir vamos classificar cada banda em tres categorias: valores de pixel entre 0 e 50, entre 50 e 100, e finalmente entre 100 e 255. Esta função pode ser útil em caso de desejar-se extrair ou eliminar valores determinados do atributo. 
...
Outra função interessante é _dplyr::slice()_, que também permite extrair por exemplo, bandas ou valores determinados de cada dimensão.

Primeiro exemplo, extração das bandas 1 a 3.
...
Segundo exemplo, extração das colunas 50 a 100, linhas 20 a 60 das bandas 4 a 6 (infravermelho).
