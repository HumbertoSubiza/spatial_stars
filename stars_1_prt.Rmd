...
Los objetos creados son de la classe _sf_ o _simple features_ (ver documentación de la biblioteca _**sf**_). Al tratar los objetos espaciales como dataframe, sus columnas o atributos, son facilmente manipulados y extraídos sin perder el georreferenciamiento en el proceso.   
...
Sin embargo, el concepto de tener una biblioteca mas flexible y completa para el tratamiento de todo tipo de dato espacial continuaba a ser elaborada y así surge la biblioteca _**stars** - Spatiotemporal Arrays, Raster and Vector Data Cubes_

Esta biblioteca introduce otro concepto nuevo en _R_, el de _datacubes array_, que podria ser traducido como arreglo cúbico de dados (una sugerencia) y donde la altura del cubo viene dada por múltiples dimensiones.
...
Como ejemplo, matrices y dataframes pueden contener líneas y columnas de datos, la diferencia con arreglos es que éste puede tener mas de 1 dimensión, de ahi la visualización como cubos o capas superpuestas. Como limitación los datos contenidos en cada dimensión debem ser del mismo tipo
...
La biblioteca permite trabajar con dados en mallas regular o irregulamente espaciadas, así como con proyecciones cartesianas o curvilíneas. Durante la lectura e importación del archivo (o parte de él), los valores de las bandas importadas muchas vezes UINT8 o UINT16, son convertidos a valores numéricos de doble precisión y si necesario, reconvertidos. Todos los datos importados en la lectura se almacenan en una única estrutura de arreglo **stars**.

Para simplificar vamos leer um archivo raster de ejemplo que la propia biblioteca **stars** incluye. El archivo es uma imagem Landsat 7 ETM, con 6 bandas, azul, verde, rojo (BGR) y 3 más de infrarrojo, em formato Geotiff. La importación hará uso de la biblioteca GDAL.
...
Vemos que el objeto creado es de la clase **stars** y tiene 3 dimensiones, ( _x_, _y_ e _band_) y 1 atributo que son los valores de cada banda. A continuación presenta las estadísticas de los valores del atributo para la totalidad del archivo.
...
En el caso específico de la imagen importada la cantidade de células en  la dimension _x_ es de 349 y en la dimensión _y_ de 352, comenzando con los valores de 288776 para el eje _x_ y de 9120761 para _y_. El sistema de referencia es UTM zona 25S y los intervalos entre cada célula (delta) de 28,5 m y -28,5 m respectivamente. La imagen corresponde a um raster (point = F) del área de Recife, Brasil.
...
Si se quiere extrair um subconjunto de datos de una área específica, podemos usar la notación de arreglos con cuatro valores de la seguiente forma: el primer argumento en el paréntesis recto es el atributo escogido, en este caso tenemos solamente un atributo. El segundo valor se refiere a la segunda dimensión, que es _x_, el tercer valor valor se refiere a la tercera dimensión _y_ y por último, el cuarto valor se refiere a la capa o capas a extrair. Asi para extrair los valores de _x_ del 1 al 10 e _y_ del 20 al 30 de las bandas 3 y 4, colocamos el siguiente paréntesis [1, 1:10, 20:30, 3:4]. En la práctica:  
...
### 4. Leitura de archivo vectorial tipo ESRI por la biblioteca **sf**.

Los objetos del tipo vectorial pueden importarse a través de la biblioteca **sf** o pueden crearse usando funciones de la propia biblioteca **stars**. Primero vamos importar un archivo de la misma área de la imagen anterior, que representa los límites del município de Olinda, Pernambuco, Brasil. El mismo fue obtenido de la página oficial del Instituto Brasileiro de Geografia e Estatística (IBGE) <https://www.ibge.gov.br/geociencias/organizacao-do-territorio/malhas-territoriais/>.
...
El objeto es del tipo **stars**, con apenas 1 dimensión (la geometria del objeto) y 5 atributos, siendo éstos presentados en resumen estatístico, dependiendo del tipo de atributo. La visualización es semejante a la anterior, pudiendo colocar en el comando _plot()_, el nombre o el número del atributo deseado.
...
### 5. Grabación de objetos **stars**

Para finalizar esta presentación, la función _write_stars()_ permite grabar objetos al disco duro, usando la biblioteca **GDAL**. El siguiente fragmento de código extrae y graba con el nombre de "stars1.tif", las líneas 1 a 10, las columnas 20 a 30 de las bandas 4 a 6 de la imagem ETM, correspondientea a las bandas de azul, verde y rojo. A continuación lee nuevamente el archivo grabado y lo visualiza en un gráfico
...
Observese que los datos de tipo vectorial, son tratados por la biblioteca **sf**, cargada junto con la biblioteca **stars**. Las dos bibliotecas usadas em forma conjunta constituyen una poderosa herramienta par el tratamiento de este tipo de datos.

En el próximo documento veremos en forma mas detallada el manejo y tratamento de los objetos **stars**, incluindo su interacción con datos vectoriales.
...
Versión 1 de 2020-12-04 - Año Covid-19
